...
function corporatez_setup() {
	
	//Register a theme image size.
	add_theme_support( 'post-thumbnails' );
	add_image_size( 'corporatez-large-thumb', 830 );
	add_image_size( 'corporatez-medium-thumb', 550, 400, true );
	add_image_size( 'corporatez-small-thumb', 230 );
	
	//Register nav menu used in theme.
	register_nav_menus( array(
		'primary' 	=> __( 'Primary Menu', 'corporatez-ex' )
	) );
...
add_action( 'after_setup_theme', 'corporatez_setup' );

if ( ! isset( $content_width ) ) $content_width = 900;
/*--- Blog layout ---*/
function corporatez_blog_layout() {
...
function corporatez_posted_on() {
	$layout = corporatez_blog_layout();
...
function corporatez_excerpt_more($more) {
	global $post;
	$layout = corporatez_blog_layout();
	return ' ....&nbsp;&nbsp;<a class=" '.$layout.'" href="'. get_permalink($post->ID) . '">' . __('Read More', 'corporatez-ex') . '</a>';
}
add_filter('excerpt_more', 'corporatez_excerpt_more');


/*--- Enqueue scripts and styles ---*/
function corporatez_scripts() {
	...
	wp_enqueue_script( 'corporatez-common', get_template_directory_uri() . '/js/common.js', array('jquery'), '20180213', true );
	...
	wp_enqueue_style( 'corporatez-font-awesome', get_template_directory_uri() . '/fonts/font-awesome.min.css' );
...
add_action( 'wp_enqueue_scripts', 'corporatez_scripts' );

// Enqueue Bootstrap
function corporatez_enqueue_bootstrap() {
	wp_enqueue_style( 'corporatez-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true );
}
add_action( 'wp_enqueue_scripts', 'corporatez_enqueue_bootstrap', 9 );



/*--- Register widget area ---*/
add_action( 'widgets_init', 'corporatez_widgets_init' );
function corporatez_widgets_init() {
    register_sidebar( array(
        'name' => __( 'Main Sidebar', 'corporatez-ex' ),
        'id' => 'sidebar-1',
        'description' => __( 'Widgets in this area will be shown on all posts and pages.', 'corporatez-ex' ),
	...
	for ($i=1; $i<=3; $i++){
		register_sidebar( array(
			'name' => __( 'Footer-widget-area-', 'corporatez-ex' ) . $i,
			'id' => 'footer-widget-area' . $i,
			'description' => __( 'Widgets in this area will be shown on footer.', 'corporatez-ex' ),
			'before_widget' => '<li id="%1$s" class="widget %2$s">',
			'after_widget'  => '</li>',
			'before_title'  => '<h3 class="widgettitle">',
			'after_title'   => '</h3>',
...
function corporatez_edit_link() {
	edit_post_link(
		sprintf( __( '<i class="fa fa-edit"></i>Edit<span class="screen-reader-text"> "%s"</span>', 'corporatez-ex' ), get_the_title() ),
...
function corporatez_get_breadcrumb() {
    echo '<a href="'.home_url().'" rel="nofollow">'.__('Home', 'corporatez-ex').'</a>';
    if ( is_category() || is_single() ) {
        ...
        echo __("&nbsp;&nbsp;&#47;&nbsp;&nbsp;Search Results for... ", "corporatez");
        echo '"<em>';
...
function corporatez_recommended_plugin() {
...
add_action( 'tgmpa_register', 'corporatez_recommended_plugin' );
...
add_filter( 'pre_set_site_transient_update_themes', 'corporatez_check_for_update' );
function corporatez_check_for_update( $checked_data ){
...
popularfx_maybe_promo([
	'after' => 1,// In days
	'interval' => 10,// In days
	'pro_url' => 'https://www.popularfx.com/themes/wordpress/corporate/Corporatez_Pro',
	'rating' => 'https://popularfx.com/themes/wordpress/corporate/Corporatez-Ex',
	'twitter' => 'https://twitter.com/',
	'facebook' => 'https://www.facebook.com/',
	'website' => 'http://a2themes.com',
]);
