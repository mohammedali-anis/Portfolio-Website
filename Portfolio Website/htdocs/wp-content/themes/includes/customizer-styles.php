...
function twx_custom_styles( $customizer_style ) {
	...
	//Background color
	$primary_color = get_theme_mod( 'primary_color', '#175fdc' );
	$customizer_style .= '.banner-button, button, input[type="submit"], input[type="button"], input[type="reset"], .to-top, #site-navigation .sub-menu li:hover > a, .site-header.float-header  { background-color: ' . $primary_color . ' }';
	
	//Color
	$customizer_style .= 'a, h1 a, h2 a, h3 a, h4 a, h5 a, h6 a, button:hover, input[type="submit"]:hover, input[type="button"]:hover, input[type="reset"]:hover, .to-top:hover, .special .meta-post .fa, .hentry .meta-post a:hover, .special h2.title-post a:hover, .widget-section .widgettitle, .default-testimonials .client-info .client .client-name, .type-team.type-b .team-social li a, .type-team .team-content .name, .other-header .entry-title, .single-meta, #site-navigation-mobi ul > .get-pro-button > a { color: ' . $primary_color . ' }';
	...
	$customizer_style .= '.site-title a, .site-title a:hover, #site-navigation ul li a:hover, #site-navigation ul li a:hover { color: ' . esc_attr( $site_title_color ) . '; } ';
	...
	$banner_bg_pos = get_theme_mod('banner_bg_pos', 'center-center');
	$banner_bg_pos = str_replace('-',' ',$banner_bg_pos);
	$banner_height = get_theme_mod('banner_height', '700');
	$banner_bg_repeat = '';
	...
	$customizer_style .= '.header-background { ' . $banner_bg . ' background-size: ' . esc_attr($banner_bg_size) . '; background-position: ' . esc_attr($banner_bg_pos) . ';' . $banner_bg_repeat . ' height: ' . esc_attr($banner_height) . 'px; }';
	
	//Page wrapper padding
	$pg_top_padding = get_theme_mod( 'page_top_padding', '0' );
	$pg_bottom_padding = get_theme_mod( 'page_bottom_padding', '0' );
	$customizer_style .= '.page-wrap { padding-top: ' . intval($pg_top_padding) . 'px; padding-bottom: ' . intval($pg_bottom_padding) . 'px; }';
	
	//Theme text color
	$theme_text_color = get_theme_mod( 'theme_text_color', '#848991' );
